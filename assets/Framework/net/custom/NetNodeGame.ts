...
import { CallbackObject, IRequestProtocol, IResponseProtocol, NetCallFunc } from "../../libs/network/NetInterface";
import { NetNode, WebSocketReturn } from "../../libs/network/NetNode";
import { WebSock } from "../../libs/network/WebSock";
...
class GameProtocol extends NetProtocolProtobuf {
    async onHearbeat() {
        var ulq = new proto.ReqNetHearbeatMsg();
        ulq.content = "";
        var ret = await netChannel.game.req(Cmd.eNetHearbeatMsgReq, Module.Net, null!, "GameHeartResp", ulq);
        if (ret.isSucc) {
            console.log("心跳包");
        }
...
export class NetNodeGame extends NetNode {
    ...
    public req<T>(cmd: number, Modile: number, reqName: string, respName: string, params?: any): Promise<WebSocketReturn<T>> {
            ...
            var protocol: IRequestProtocol = {
                ...
                callback: {
                    ...
                    callback: (iresp: IResponseProtocol) => {
                        ...
                        if (res.isSucc) {
                            res.res = iresp.data;
                            console.log("收到服务器【" + cmd + "】协议返回数据", iresp.data);
                        }
    ...
    public reqUnique<T>(cmd: number, Modile: number, reqName: string, respName: string, params: any): Promise<WebSocketReturn<T>> {
        return new Promise((resolve, reject) => {
            var protocol: IRequestProtocol = {
                cmd: cmd,
                Modile: Modile,
     ...
     * @param target    目标对象
     */
    on(cmd: number, Modile: number, respName: string, callback: NetCallFunc, target?: any): void {
