...
export class Root extends Component {
    ...
    private init() {
        ...
        if (!sys.isMobile) {
            screen.on("window-resize", () => {
                oops.message.emit(EventMessage.GAME_RESIZE);
            }, this);

            screen.on("fullscreen-change", () => {
                oops.message.emit(EventMessage.GAME_FULL_SCREEN);
            }, this);
        }

        screen.on("orientation-change", () => {
            oops.message.emit(EventMessage.GAME_ORIENTATION);
        }, this);
    }
    ...
    private onShow() {
        ...
        oops.message.emit(EventMessage.GAME_SHOW);
    }
    ...
    private onHide() {
        ...
        oops.message.emit(EventMessage.GAME_HIDE);
    }
}